...
func printAssets(repo *assets.Folder, depLocation string) (printed string) {
	...
	for _, repo := range repo.Dependencies {
		...
		printed = printed + innerLocation + ".Files = make(map[string]*achaemenid.AssetsFile)" + "\n"
		printed = printed + innerLocation + ".Dependencies = make(map[string]*achaemenid.Assets)" + "\n"
...
func printFile(fileName string, fileDate []byte) (print string) {
	...
	print = fmt.Sprintf(`&achaemenid.AssetsFile{Name: "%s", Data: %s}`, fileName, data)
...
var Assets = achaemenid.NewAssets()

func init(){
	achaemenid.DefaultServer.Assets = Assets.Dependencies["assets"]

	Assets.Dependencies["assets"].Files = make(map[string]*achaemenid.AssetsFile)
	Assets.Dependencies["assets"].Dependencies = make(map[string]*achaemenid.Assets)
	{{range .Dependencies}}
		{{if (eq .Name "assets")}} 
