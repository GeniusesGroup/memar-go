...
type BlockVolatile interface {
	...
	Extend(cap adt_p.NumberOfElement) (extended adt_p.NumberOfElement, err protocol.Error)

	// Change the return data not flush to any non-volatile storages. Use Write() to change data.
	Read(offset adt_p.ElementIndex, limit adt_p.NumberOfElement, buf buffer_p.Buffer) (err protocol.Error)
	// Write at more than block capacity cause block extend. extend capacity isn't equal to write length.
	Write(offset adt_p.ElementIndex, buf buffer_p.Buffer) (err protocol.Error)
	Search(data []byte, offset adt_p.ElementIndex) (loc adt_p.ElementIndex, err protocol.Error)

	Erase(offset adt_p.ElementIndex, limit adt_p.NumberOfElement) (err protocol.Error)
	Copy(desOffset, srcOffset adt_p.ElementIndex, limit adt_p.NumberOfElement) (err protocol.Error)
	Move(desOffset, srcOffset adt_p.ElementIndex, limit adt_p.NumberOfElement) (err protocol.Error)
}
