...
func (ser *writeService) ServeSRPC(st protocol.Stream, srpcReq protocol.SRPCRequest) (res protocol.Syllab, err protocol.Error) {
	var srpcRequestPayload = srpcReq.Payload()
	var reqAsSyllab = writeRequestSyllab(srpcRequestPayload)
	...
	// Do for local node
	err = protocol.OS.ObjectDirectory().Write(reqAsSyllab.ObjectID(), reqAsSyllab.ObjectStructureID(), reqAsSyllab.Offset(), reqAsSyllab.Data())
	return
}

func write(req *WriteReq) (err protocol.Error) {
	...
	// Do for local node
	err = protocol.OS.ObjectDirectory().Write(req.objectID, req.objectStructureID, req.offset, req.data)
	return
}
