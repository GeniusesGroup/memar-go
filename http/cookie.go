...
// ParseCookie parse cookie values to c and return!
func (c *Cookie) ParseCookie(cookie string) {
	...
	for i := 0; i < len(parts); i++ {
		...
		c.Name, c.Value = parts[i], ""
		if j := strings.Index(c.Name, "="); j >= 0 {
			c.Name, c.Value = c.Name[:j], c.Name[j+1:]
		}
		if !isCookieNameValid(c.Name) {
			continue
		}
		c.Value, _ = parseCookieValue(c.Value, true)
	}
}
