...
func (p *LIFO) Start() {
	...
	if pStatus != PoolStatus_Unset {
		if pStatus == PoolStatus_Running {
			panic("[pool] LIFO instance already started")
		}
		if pStatus == PoolStatus_Stopping {
			panic("[pool] LIFO instance is on stopping state and can't re-start before last stop proccess")
		}
...
func (p *LIFO) Stop() {
	...
	if wpStatus != PoolStatus_Running {
		panic("[pool] LIFO instance wasn't started")
	}
