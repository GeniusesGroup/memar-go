...
type StorageObjects interface {
	...
	ListObjects(mt MediaTypeID, offset, limit uint64) (ids [][16]byte, err Error)

	Lock(mt MediaTypeID, id [16]byte) (err Error)
	Unlock(mt MediaTypeID, id [16]byte) (err Error)

	Length(mt MediaTypeID, id [16]byte) (ln int, err Error)
	Get(mt MediaTypeID, id [16]byte) (object []byte, err Error)
	Read(mt MediaTypeID, id [16]byte, offset, limit uint64) (data []byte, err Error)
	Save(mt MediaTypeID, id [16]byte, object []byte) (err Error)
	Write(mt MediaTypeID, id [16]byte, offset uint64, data []byte) (err Error)

	Append(mt MediaTypeID, id [16]byte, data []byte) (err Error)
	Prepend(mt MediaTypeID, id [16]byte, data []byte) (err Error)
	Extend(mt MediaTypeID, id [16]byte, length uint64) (err Error)
	...
	Erase(mt MediaTypeID, id [16]byte) (err Error)
